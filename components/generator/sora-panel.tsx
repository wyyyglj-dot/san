...
export function SoraPanel({ onTaskAdded }: SoraPanelProps) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      // 提交任务
      const data = await apiPost<{ id: string }>('/api/generate/sora', {
        model: taskModel,
        prompt: taskPrompt,
      ...
      setError(err instanceof ApiClientError ? err.message : '生成失败');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="p-6 space-y-6">
              ...
              <button
                ...
                className={cn(
                  ...
                  ratio === r.value
                    ...
                    : 'bg-card/40 text-foreground/55 border-white/[0.06] hover:bg-card/65 hover:text-foreground'
                )}
              >
        ...
        <div className="space-y-3">
          ...
          {files.length === 0 ? (
            <div
              ...
              className="border border-dashed border-white/[0.06] rounded-xl p-8 text-center cursor-pointer hover:bg-card/50 hover:border-white/[0.1] transition-all"
            >
        ...
        <div className="space-y-3">
          ...
          <textarea
            ...
            className="w-full h-28 px-4 py-3 bg-input/50 border border-white/[0.06] text-foreground rounded-xl resize-none focus:outline-none focus:border-white/[0.1] focus:ring-2 focus:ring-ring/30 placeholder:text-muted-foreground/60 text-sm"
          />
          <label className="flex items-center gap-2 cursor-pointer select-none">
            <input
              ...
              className="w-4 h-4 rounded border-white/[0.06] bg-card/40 text-foreground accent-sky-400 cursor-pointer"
            />
