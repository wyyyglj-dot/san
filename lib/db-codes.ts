...
export async function getInviteCodes(options: {
  ...
  sql = appendLimitOffset(sql + ' ORDER BY created_at DESC', params, limit, offset);
...
export async function getAllGenerations(options: {
  ...
  let genSql = `SELECT g.*, u.email as user_email, u.name as user_name
     FROM generations g
     LEFT JOIN users u ON g.user_id = u.id
     ${whereStr}
     ORDER BY g.created_at DESC`;
  genSql = appendLimitOffset(genSql, params, limit, offset);
  const [rows] = await db.execute(genSql, params);
