...
import { generateWithGrokVideo } from './grok-video-api';
...
export async function generateVideo(
  ...
  } else if (channel.type === 'grok-video' && inputImages.length > 2) {
    const sliced = inputImages.slice(0, 5);
  ...
  switch (channel.type) {
    ...
    case 'jimeng':
      ...
      break;
    case 'grok-video':
      result = await generateWithGrokVideo(videoRequest, onProgress, {
        baseUrl: effectiveBaseUrl,
        apiKey: effectiveApiKey,
        channelId: channel.id,
        onVideoIdReady: options?.onVideoIdReady,
