...
type RateLimitConfigType = {
  [K in keyof typeof RateLimitConfig]: { maxRequests: number; windowSeconds: number };
};

// 从数据库获取动态限流配置
export async function getRateLimitConfig(): Promise<RateLimitConfigType> {
  const config = await getSystemConfig();
  const r = config.rateLimit;
  const w = r?.windowSeconds ?? 60;
  return {
    API: { maxRequests: r?.api ?? 60, windowSeconds: w },
    GENERATE: { maxRequests: r?.generate ?? 20, windowSeconds: w },
    CHAT: { maxRequests: r?.chat ?? 30, windowSeconds: w },
    AUTH: { maxRequests: r?.auth ?? 5, windowSeconds: w },
  };
}
...
export async function checkRateLimitAsync(
  ...
  keyPrefix = 'api'
): Promise<{ allowed: boolean; remaining: number; resetAt: number; headers: Record<string, string> }> {
  const config = await getRateLimitConfig();
  return checkRateLimit(request, config[configKey], keyPrefix);
}
