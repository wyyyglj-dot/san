...
export default function StatsPage() {
  ...
  return (
    <AdminPageLayout
      title="数据统计"
      description="系统运行数据概览"
      icon={BarChart3}
      rightElement={
        <select
          value={days}
          onChange={(e) => setDays(Number(e.target.value))}
          className="px-4 py-2 bg-card/50 border border-white/[0.06] rounded-xl text-foreground focus:outline-none focus:border-white/[0.06] [&>option]:bg-card/85 [&>option]:text-foreground"
        >
        ...
        {stats.dailyStats.length === 0 ? (
          ...
          <div className="flex">
            ...
            <div className="flex-1 flex flex-col">
              <div className="h-48 flex items-end gap-[2px] border-l border-b border-white/[0.06] pl-1">
          ...
          <table className="w-full min-w-[640px]">
            ...
            <tbody>
              {[...stats.dailyStats].reverse().map((day) => (
                <tr key={day.date} className="border-b border-white/[0.06] hover:bg-card/40">
