...
import { History, Trash2, Search, Loader2, Eye, Sparkles } from 'lucide-react';
...
export default function GenerationsPage() {
  ...
  return (
    <AdminPageLayout title="生成记录" description={`管理所有用户的生成历史 · 共 ${total} 条`} icon={Sparkles}>
      {/* Filters */}
      <div className="flex flex-wrap gap-3">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            ...
            className="w-full pl-11 pr-4 py-3 bg-card/40 border border-white/[0.06] rounded-xl text-foreground placeholder:text-foreground/30 focus:outline-none focus:border-white/[0.06]"
          />
        </div>
        <select
          value={typeFilter}
          onChange={(e) => setTypeFilter(e.target.value)}
          className="px-4 py-3 bg-card/40 border border-white/[0.06] rounded-xl text-foreground focus:outline-none focus:border-white/[0.06]"
        >
          ...
          <table className="w-full min-w-[760px]">
            ...
            <tbody>
              {filteredRecords.map((record) => (
                <tr key={record.id} className="border-b border-white/[0.06] hover:bg-card/40">
                  ...
                  <td className="px-5 py-4">
                    <span className="px-2 py-1 text-xs rounded-full bg-card/50 text-foreground/70">
      ...
      {hasMore && (
        <button
          ...
          className="w-full py-3 bg-card/40 border border-white/[0.06] text-foreground/60 rounded-xl hover:bg-card/50 transition-all"
        >
          加载更多
        </button>
      )}
