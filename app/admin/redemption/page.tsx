...
import { Ticket, Plus, Trash2, Copy, Loader2, Check, Gift } from 'lucide-react';
      ...
      rightElement={
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button
            ...
            className="px-4 py-2 bg-card/40 border border-white/[0.06] text-foreground rounded-xl hover:bg-card/50 transition-all"
          >
            复制全部
          </button>
          <button
    ...
    >
      ...
      {showCreate && (
        <div className="fixed inset-0 bg-background/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-card/85 border border-white/[0.06] rounded-2xl p-6 w-full max-w-md">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  min={1}
                  max={100}
                  className="w-full px-4 py-3 bg-card/40 border border-white/[0.06] rounded-xl text-foreground focus:outline-none focus:border-white/[0.06]"
                />
              </div>
              <div>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="如：活动赠送"
                  className="w-full px-4 py-3 bg-card/40 border border-white/[0.06] rounded-xl text-foreground placeholder:text-foreground/30 focus:outline-none focus:border-white/[0.06]"
                />
              </div>
            </div>
            ...
            <div className="flex gap-3 mt-6">
              <button
                ...
                className="flex-1 py-3 bg-card/40 border border-white/[0.06] text-foreground rounded-xl hover:bg-card/50 transition-all"
              >
                取消
              </button>
              <button
      ...
      <div className="bg-card/40 border border-white/[0.06] rounded-2xl overflow-hidden">
        <div className="overflow-x-auto no-scrollbar">
          <table className="w-full min-w-[760px]">
            ...
            <tbody>
              {codes.map((code) => (
                <tr key={code.id} className="border-b border-white/[0.06] hover:bg-card/40">
                  <td className="px-5 py-4">
                    <code className="font-mono text-foreground bg-card/40 px-2 py-1 rounded">
                      {code.code}
                    </code>
                  </td>
                  ...
                  <td className="px-5 py-4 text-center">
                    {code.usedBy ? (
                      <span className="px-2 py-1 text-xs rounded-full bg-card/50 text-foreground/50">
                        已使用
                      </span>
                    ) : (
                  ...
                  <td className="px-5 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button
                        ...
                        className="p-2 text-foreground/40 hover:text-foreground hover:bg-card/50 rounded-lg transition-all"
                      >
                        {copiedId === code.id ? (
