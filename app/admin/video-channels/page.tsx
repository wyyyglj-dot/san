...
const CHANNEL_TYPES: { value: ChannelType; label: string }[] = [
  ...
  { value: 'grok-video', label: 'Grok 视频' },
  { value: 'openai-compatible', label: 'OpenAI 兼容' },
  { value: 'kie-ai', label: 'kie.ai 原生' },
  { value: 'suchuang', label: '速创(Wuyin)' },
...
export default function VideoChannelsPage() {
  ...
  const resetChannelForm = () => {
    setChannelForm({ name: '', type: 'sora', baseUrl: '', apiKey: '', proxyUrl: '', enabled: true, isListed: true });
  ...
  const resetModelForm = () => {
    setModelForm({
      ...
      highlight: false, hdEnabled: false, enabled: true, sortOrder: 0,
    });
  ...
  const startEditChannel = (channel: VideoChannel) => {
    setChannelForm({
      ...
      isListed: channel.isListed !== undefined ? channel.isListed : true,
    });
  ...
  const startEditModel = (model: VideoModel) => {
    setModelForm({
      ...
      hdEnabled: model.hdEnabled || false,
  ...
  const toggleChannelListed = async (channel: VideoChannel) => {
    try {
      const res = await fetch('/api/admin/video-channels', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      <div className="bg-card/40 border border-white/[0.06] rounded-2xl p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            ...
            >
              {CHANNEL_TYPES.map(t => (
                <option key={t.value} value={t.value} className="bg-card/85">{t.label}</option>
              ))}
            </select>
          </div>
          <div className="space-y-2">
            <label className="text-sm text-foreground/70">接口地址</label>
            <input
              type="text"
          ...
          <div className="space-y-2">
            <label className="text-sm text-foreground/70">API 密钥</label>
          ...
          <div className="space-y-2 md:col-span-2">
            <label className="text-sm text-foreground/70">代理地址 (可选，覆盖全局设置)</label>
            <input
              type="text"
        ...
        <div className="flex items-center gap-6 pt-2">
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            />
            <span className="text-sm text-foreground/70">前端可见</span>
          </label>
        </div>
        ...
        <div className="bg-card/40 border border-white/[0.06] rounded-2xl p-6 space-y-4">
          ...
          <div className="space-y-3 pt-2">
            ...
            <div className="flex flex-wrap gap-4">
              {[
                ...
                { key: 'referenceToVideo', label: '双图参考 (R2V)' },
                { key: 'videoToVideo', label: '视频转视频' },
                { key: 'supportStyles', label: '支持风格' },
                { key: 'characterCreation', label: '角色创建' },
                { key: 'omniReference', label: '即梦-多主体' },
              ].map(f => (
          ...
          <div className="flex flex-wrap gap-4 pt-2">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                checked={modelForm.hdEnabled}
                onChange={(e) => setModelForm({ ...modelForm, hdEnabled: e.target.checked })}
                className="w-4 h-4 rounded border-white/[0.06] bg-card/40 text-blue-500 focus:ring-blue-500"
              />
              <span className="text-sm text-foreground/70">高清模式 (1080p)</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
              <input
                type="checkbox"
                  ...
                  <div className="p-4 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        title={channel.isListed ? '前端可见' : '前端隐藏'}
                      >
                        {channel.isListed ? '可见' : '隐藏'}
                      </button>
                            ...
                            <div className="flex items-center gap-2">
                              <button
                                ...
                                className={`px-2 py-0.5 text-xs rounded-full ${
                                  model.enabled ? 'bg-green-500/20 text-green-400' : 'bg-card/50 text-foreground/40'
                                }`}
                              >
                                {model.enabled ? '启用' : '禁用'}
                              </button>
