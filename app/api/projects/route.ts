...
import { createComicProject, getComicProjects, createProjectPreferences, purgeComicProject } from '@/lib/db-comic';
...
export const POST = authHandler(async (req, _ctx, session) => {
    ...
    // Validate defaultEra if provided
    const ALLOWED_ERAS = ['modern', 'ancient', 'medieval'];
    const rawEra = parseOptionalString(preferencesInput?.defaultEra);
    if (rawEra && !ALLOWED_ERAS.includes(rawEra)) {
    ...
    try {
      await createProjectPreferences(project.id, {
        defaultImageModelId: parseOptionalString(preferencesInput?.defaultImageModelId),
        defaultVideoModelId: parseOptionalString(preferencesInput?.defaultVideoModelId),
        defaultTextModelId: parseOptionalString(preferencesInput?.defaultTextModelId),
        defaultStyle: parseOptionalString(preferencesInput?.defaultStyle),
        defaultEra: rawEra,
        defaultVideoRatio: typeof preferencesInput?.defaultVideoRatio === 'string'
          ? preferencesInput.defaultVideoRatio.trim() || '16:9'
          : '16:9',
      });
    } catch (prefError) {
      // Rollback project creation if preferences fail
      try {
