...
export const PATCH = authHandler(async (req, ctx, session) => {
    ...
    if (preferencesInput) {
      const prefUpdates: Parameters<typeof updateProjectPreferences>[1] = {};
      if (preferencesInput.defaultImageModelId !== undefined) {
        prefUpdates.defaultImageModelId = preferencesInput.defaultImageModelId === null
          ? null : String(preferencesInput.defaultImageModelId).trim() || null;
      }
      if (preferencesInput.defaultVideoModelId !== undefined) {
        prefUpdates.defaultVideoModelId = preferencesInput.defaultVideoModelId === null
          ? null : String(preferencesInput.defaultVideoModelId).trim() || null;
      }
      if (preferencesInput.defaultTextModelId !== undefined) {
        prefUpdates.defaultTextModelId = preferencesInput.defaultTextModelId === null
          ? null : String(preferencesInput.defaultTextModelId).trim() || null;
      }
      if (preferencesInput.defaultStyle !== undefined) {
        prefUpdates.defaultStyle = preferencesInput.defaultStyle === null
          ? null : String(preferencesInput.defaultStyle).trim() || null;
      }
      if (preferencesInput.defaultVideoRatio !== undefined) {
        const ratio = String(preferencesInput.defaultVideoRatio).trim();
        if (ratio) prefUpdates.defaultVideoRatio = ratio;
      }
      const ALLOWED_ERAS = ['modern', 'ancient', 'medieval'];
      if (preferencesInput.defaultEra !== undefined) {
        const era = preferencesInput.defaultEra === null
          ? null : String(preferencesInput.defaultEra).trim() || null;
        if (era !== null && !ALLOWED_ERAS.includes(era)) {
          return NextResponse.json({ error: '无效的时代设定' }, { status: 400 });
        }
        prefUpdates.defaultEra = era;
      }
      await updateProjectPreferences(id, prefUpdates);
    }
