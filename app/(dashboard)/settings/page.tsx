...
export default function SettingsPage() {
  ...
  const handleRedeemCode = async () => {
    ...
    try {
      ...
      notify.success('兑换成功', `获得 ${formatBalance(data.points)} 积分`);
      setRedeemCode('');
      updateSession();
    } catch (err) {
      notify.fromError(err, '兑换失败');
    } finally {
  ...
  const handleUseInviteCode = async () => {
    ...
    try {
      ...
      notify.success('使用成功', `获得 ${formatBalance(data.bonusPoints)} 积分`);
      setInviteCode('');
      updateSession();
    } catch (err) {
      notify.fromError(err, '使用失败');
    } finally {
              ...
              <p className="text-foreground text-lg">
                {session.user.role === 'admin' ? (
                  <span className="inline-flex items-center gap-2">
                    管理员
                    <span className="px-2 py-0.5 bg-card/50 text-foreground/60 text-xs rounded border border-white/[0.05]">Admin</span>
                  </span>
                ) : '普通用户'}
              </p>
            </div>
        ...
        <div className="p-6 space-y-4">
          <div className="flex gap-3">
            <input
              ...
              placeholder="输入兑换码"
              className="flex-1 px-4 py-3 bg-input/50 border border-white/[0.06] rounded-xl text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:border-white/[0.1] focus:ring-2 focus:ring-ring/30 transition-colors uppercase tracking-wider"
            />
            <button
      ...
      <div className="surface overflow-hidden">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-3">
              <div className="flex-1 px-4 py-3 bg-card/40 border border-white/[0.06] rounded-xl text-foreground font-mono tracking-wider flex items-center">
                {inviteCodeLoading ? (
              ...
              {myInviteCode && (
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-3 bg-card/40 border border-white/[0.06] text-foreground/60 rounded-xl hover:bg-card/65 hover:text-foreground transition-colors"
                >
        ...
        <div className="p-6 space-y-5">
          <div className="space-y-2">
            ...
            <input
              ...
              placeholder="输入当前密码"
              className="w-full px-4 py-3 bg-input/50 border border-white/[0.06] rounded-xl text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:border-white/[0.1] focus:ring-2 focus:ring-ring/30 transition-colors"
            />
          </div>
          <div className="space-y-2">
