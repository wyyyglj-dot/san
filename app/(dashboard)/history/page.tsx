...
import { getPollingInterval, shouldContinuePolling, isHardTimeout, type TaskType } from '@/lib/polling-utils';
import { formatDate, truncate } from '@/lib/utils';
import { downloadAsset } from '@/lib/download';
import { IMAGE_MODELS } from '@/lib/model-config';
import { TimestampSelectorModal } from '@/components/generator/timestamp-selector-modal';
...
// 骨架屏组件
const SkeletonCard = () => (
  <div className="relative aspect-video bg-card/40 rounded-xl overflow-hidden border border-white/[0.06] animate-pulse">
    <div className="absolute inset-0 bg-gradient-to-br from-white/5 to-white/10" />
    <div className="absolute top-2 right-2 w-16 h-5 bg-card/50 rounded-md" />
    <div className="absolute bottom-0 left-0 right-0 p-3 space-y-2">
      <div className="h-3 bg-card/50 rounded w-3/4" />
      <div className="h-2 bg-card/50 rounded w-1/3" />
    </div>
  </div>
);
    ...
    <div className="min-w-0">
      ...
      <div className="mt-2 flex items-center gap-2">
        <button
          ...
          className="text-xs text-foreground/50 hover:text-foreground/65 hover:underline underline-offset-4 transition-colors"
          type="button"
        >
          {expanded ? '收起' : '展开'}
        </button>
        <button
          ...
          className="inline-flex items-center gap-1 text-xs text-foreground/50 hover:text-foreground/65 transition-colors"
          title="复制提示词"
          type="button"
        >
    ...
    <div
      className={`group relative aspect-video bg-card/40 rounded-xl overflow-hidden cursor-pointer border transition-all ${
        isSelected 
          ...
          : 'border-white/[0.06] hover:border-border'
      }`}
      onClick={handleClick}
    >
      {isVideoType(gen) ? (
        ...
        <>
          {!imageLoaded && (
            <div className="absolute inset-0 bg-card/40 animate-pulse" />
          )}
          <img
            src={gen.resultUrl}
            alt={gen.prompt}
      ...
      {!selectMode && (
        <div className="absolute inset-0 bg-background/70 opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center">
          <div className="w-12 h-12 bg-card/50 border border-white/[0.06] backdrop-blur-sm rounded-full flex items-center justify-center">
...
export default function HistoryPage() {
  ...
  const pollTaskStatus = useCallback(async (taskId: string, taskType: TaskType = 'image') => {
    // 防止重复轮询
    ...
    const poll = async () => {
      ...
      if (!shouldContinuePolling(elapsed, taskType)) {
        // 视频任务：软超时后继续降频轮询，由后端决定最终状态
        if (taskType === 'video' && !isHardTimeout(elapsed, taskType)) {
          const interval = getPollingInterval(elapsed, taskType);
          setTimeout(poll, interval);
          return;
        }
        toast({
          title: '任务超时',
          description: '轮询已停止，任务可能仍在后台处理中',
        });
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (tasks.length > 0) {
          ...
          tasks.forEach(task => {
            const taskType: TaskType = task.type?.includes('video') ? 'video' : 'image';
            pollTaskStatus(task.id, taskType);
          });
        }
      }
    } catch (err) {
  ...
  const handleCreateCharacter = async (start: number, end: number, extra?: { characterPrompt?: string; characterUserName?: string; firstFrameBase64?: string }) => {
    ...
    try {
      const res = await fetch('/api/generate/character-card/from-task', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
    ...
    <>
      <div className="max-w-7xl mx-auto space-y-8">
          ...
          <div className="flex flex-wrap items-center gap-4 md:gap-6">
            {stats.pending > 0 && (
              <>
                ...
                <div className="hidden md:block w-px h-8 bg-card/50" />
              </>
            )}
            <div className="text-center min-w-[72px]">
              ...
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm transition-all ${
                  filter === f
                    ...
                    : 'bg-card/40 text-foreground/60 hover:bg-card/50 hover:text-foreground'
                }`}
              >
            ...
            {selectMode ? (
              <>
                <button
                  ...
                  className="flex items-center gap-2 px-3 py-2 w-full sm:w-auto bg-card/40 text-foreground/60 rounded-lg text-sm hover:bg-card/50 hover:text-foreground transition-all"
                >
              ...
              <>
                <button
                  ...
                  className="flex items-center gap-2 px-3 py-2 w-full sm:w-auto bg-card/40 text-foreground/60 rounded-lg text-sm hover:bg-card/50 hover:text-foreground transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
        ...
        <div className="bg-card/40 border border-white/[0.06] rounded-2xl overflow-hidden backdrop-blur-sm">
          <div className={`p-4 sm:p-6 border-b border-white/[0.06] ${filter === 'character' ? 'bg-gradient-to-r from-emerald-500/10 to-sky-500/10' : 'bg-gradient-to-r from-sky-500/10 to-emerald-500/10'}`}>
          ...
          <div className="p-4 sm:p-6">
            {loading ? (
              ...
              completedCharacterCards.length === 0 && processingCharacterCards.length === 0 ? (
                <div className="h-64 flex flex-col items-center justify-center border border-dashed border-white/[0.06] rounded-xl">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                  ...
                  {processingCharacterCards.map((card) => (
                    <div
                      key={card.id}
                      className="bg-card/40 border border-sky-500/30 rounded-xl overflow-hidden"
                    >
                ...
                {hasHiddenGenerations && (
                  <div ref={renderMoreRef} className="mt-6 flex items-center justify-center">
                    <button
                      ...
                      className="px-4 py-2 rounded-lg bg-card/40 border border-white/[0.06] text-foreground/70 text-sm hover:text-foreground hover:border-border transition"
                    >
                      Load more
                    </button>
                  </div>
              ...
              <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap items-center gap-2 md:gap-3 mt-2">
                    ...
                    <span className="text-foreground/30 hidden md:inline">·</span>
                    <span className="px-2 py-0.5 bg-card/50 text-foreground/60 text-xs rounded">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      {selected.resultUrl && (
                        <button
                          ...
                          className="shrink-0 p-1.5 text-foreground/40 hover:text-foreground/65 hover:bg-card/50 rounded-lg transition-colors"
                          title="复制 URL"
                        >
                ...
                <div className="flex gap-2 shrink-0 w-full md:w-auto">
                  {isVideoType(selected) && selected.params?.videoId && (
                    <button
                  ...
                  <button
                    onClick={() => setSelected(null)}
                    className="flex items-center justify-center gap-2 px-5 py-2.5 bg-card/50 text-foreground border border-white/[0.06] rounded-xl hover:bg-card/65 transition-colors text-sm font-medium"
                  >
                    <X className="w-4 h-4" />
                    关闭
                  </button>
      ...
      {showDeleteConfirm && (
        ...
        >
          <div
            className="bg-card/85 border border-white/[0.06] rounded-2xl p-6 max-w-md w-full"
          ...
          >
            ...
            <div className="flex gap-3">
              <button
                onClick={() => setShowDeleteConfirm(null)}
                disabled={deleting}
                className="flex-1 px-4 py-2.5 bg-card/40 text-foreground border border-white/[0.06] rounded-xl hover:bg-card/50 transition-colors text-sm font-medium disabled:opacity-50"
              >
                取消
              </button>
              <button
  ...
  return (
    <div className="group relative bg-gradient-to-br from-emerald-500/5 to-sky-500/5 rounded-xl overflow-hidden border border-emerald-500/20 hover:border-emerald-500/40 transition-all">
      ...
      <div className="absolute inset-0 bg-gradient-to-br from-emerald-500/20 to-sky-500/20 opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center">
        <div className="w-12 h-12 bg-card/50 backdrop-blur-sm rounded-full flex items-center justify-center">
