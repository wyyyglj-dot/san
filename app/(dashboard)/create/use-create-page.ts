...
import type { OmniSubject } from './omni-reference-panel';
import type {
  SafeImageChannel,
  SafeVideoChannel,
  DailyLimitConfig,
...
export function useCreatePage() {
  ...
  // Omni reference state (即梦多主体)
  const [omniSubjects, setOmniSubjects] = useState<OmniSubject[]>([]);
      ...
      for (let i = 0; i < gachaCount; i++) {
        ...
        if (mediaType === 'image') {
          ...
          const payload = {
            ...
            files: currentVideoModel?.features.omniReference && omniSubjects.length > 0
              ? omniSubjects.filter((subject) => subject.file).map((subject) => ({ mimeType: subject.file!.mimeType, data: subject.file!.data }))
              : compressedFiles,
            style_id: currentVideoModel?.features.supportStyles ? selectedStyle || undefined : undefined,
            functionMode: (currentVideoModel?.features.omniReference && omniSubjects.length > 0)
              ? 'omni_reference' as const
              : undefined,
          };
